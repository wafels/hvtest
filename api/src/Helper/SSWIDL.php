    ...
    private function _getProvenanceComment() {
        ...
        if ( array_key_exists('movieId',$this->_params) && !is_null($this->_params['movieId']) ) {
            $comment .= "to download the original science data used to generate\n" 
                     .  "; the Helioviewer.org movie http://helioviewer.org/?movieId="
...
a_{$layer['instrument']}_{$layer['measurement']} = vso_search(tstart, tend, instrument='{$layer['instrument']}', wave='{$layer['measurement']}')
b_{$layer['instrument']}_{$layer['measurement']} = vso_get( a_{$layer['instrument']}_{$layer['measurement']} )
...
a_{$layer['instrument']}_{$layer['detector']} = vso_search(tstart, tend, instrument='{$layer['instrument']}', detector='{$layer['detector']}')
b_{$layer['instrument']}_{$layer['detector']} = vso_get( a_{$layer['instrument']}_{$layer['detector']} )
...
a_{$layer['instrument']}_{$layer['measurement']} = vso_search(tstart, tend, instrument='{$layer['instrument']}'{$physobs_str})
b_{$layer['instrument']}_{$layer['measurement']} = vso_get( a_{$layer['instrument']}_{$layer['measurement']} )
...
EOD;
        $count = 0;
        foreach ( $this->_imageLayers as $i=>$layer ) {
            $observatory = str_replace('-','',$layer['observatory']);
...
a_{$observatory}_{$layer['instrument']}_{$layer['detector']}_{$layer['measurement']} = vso_search(tstart, tend, $
    source='{$layer['observatory']}', instrument='{$layer['instrument']}', $
    detector='{$layer['detector']}', wave='{$layer['measurement']}')
b_{$observatory}_{$layer['instrument']}_{$layer['detector']}_{$layer['measurement']} = vso_get( a_{$observatory}_{$layer['instrument']}_{$layer['detector']}_{$layer['measurement']} )
...
a_{$observatory}_{$layer['instrument']}_{$layer['detector']} = vso_search(tstart, tend, $
    source='{$layer['observatory']}', instrument='{$layer['instrument']}', detector='{$layer['detector']}')
b_{$observatory}_{$layer['instrument']}_{$layer['detector']} = vso_get( a_{$observatory}_{$layer['instrument']}_{$layer['detector']} )
...
a_{$layer['instrument']} = vso_search(tstart, tend, instrument='SWAP', wave='{$layer['measurement']}')
b_{$layer['instrument']} = vso_get( a_{$layer['instrument']} )
...
a_{$layer['instrument']} = vso_search(tstart, tend, source='YOHKOH', instrument='{$layer['instrument']}')
b_{$layer['instrument']} = vso_get( a_{$layer['instrument']} )
    ...
    private function _getSDOSnippet() {
        ...
        foreach ( $this->_imageLayers as $i=>$layer ) {
            if ( $layer['instrument'] == 'AIA' ) {
                ...
                if ( $layer['measurement'] == 'magnetogram' ) {
                    $wave = 'mag';
                }
                else if ( $layer['measurement'] == 'continuum' ) {
                    $wave = 'cont';
                }                
